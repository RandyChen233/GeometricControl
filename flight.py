...
class SimpleClient:
    ...
    def connected(self, uri):
        ...
        for v in variables:
            num_variables += 1
            # <-- could increase if you paid attention to types / sizes (max 30 bytes per packet)
            if num_variables > 5:
                num_variables = 0
                self.logconfs.append(
                    LogConfig(name=f'LogConf{len(self.logconfs)}', period_in_ms=10))
        ...
        for logconf in self.logconfs:
            try:
                ...
                print(
                    f'Could not start {logconf.name} because of bad configuration')
                for v in logconf.variables:
                    print(f' - {v.name}')
