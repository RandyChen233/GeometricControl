...
class Simulator:
    ...
    def __init__(
        ...
        if self.display:
            pybullet.connect(
                pybullet.GUI, options=f'--width={width} --height={height}')
    ...
    def camera(self):
        if self.display:
            if isinstance(self.camera_target, str):
                ...
                if drone is None:
                    raise Exception(
                        f'drone "{self.camera_target}" does not exist')
                pos, ori = pybullet.getBasePositionAndOrientation(drone['id'])
                eul = pybullet.getEulerFromQuaternion(ori)
                pybullet.resetDebugVisualizerCamera(
                    self.camera_distance, (eul[2] * 180 / np.pi) - 90 + self.camera_yaw, -self.camera_pitch, pos)
            else:
                pybullet.resetDebugVisualizerCamera(
                    self.camera_distance, -90 + self.camera_yaw, -self.camera_pitch, self.camera_target)
    ...
    def reset(self):
        ...
        for drone in self.drones:
            try:
                ...
                print(
                    f'Failed to initialize drone {drone["name"]} (turning it off):')
    ...
    def get_state(self, drone):
        ...
        R_body_in_world = np.reshape(
            np.array(pybullet.getMatrixFromQuaternion(ori)), (3, 3))
    ...
    def set_state(self, name, state):
        ...
        pybullet.resetBasePositionAndOrientation(
            drone['setpoint_frame_id'], pos, ori)
    ...
    def run(self, max_time=None, data_filename=None, video_filename=None):
        ...
        if elapsed_time > 0:
            print(
                f'Completed {elapsed_time_steps} time steps in {elapsed_time:.4f} seconds ({(elapsed_time_steps / elapsed_time):.4f} time steps per second)')
    ...
    def step(self):
        ...
        for index, drone in enumerate(self.drones):
            ...
            try:
                ...
                pos = np.array(
                    [setpoint['o_x'], setpoint['o_y'], setpoint['o_z']])
            ...
            # apply rotor forces
            pybullet.applyExternalForce(drone['id'], 0, np.array(
                [0., 0., drone['u'][3]]), np.array([0., 0., 0.]), pybullet.LINK_FRAME)

            # apply rotor torques
            pybullet.applyExternalTorque(drone['id'], 0, np.array(
                [drone['u'][0], drone['u'][1], drone['u'][2]]), pybullet.LINK_FRAME)
    ...
    def snapshot(self):
        ...
        view_matrix = pybullet.computeViewMatrixFromYawPitchRoll(
            pos, self.camera_distance, yaw, -self.camera_pitch, 0., 2)
        projection_matrix = pybullet.computeProjectionMatrixFOV(
            fov=120, aspect=1.0, nearVal=0.01, farVal=100.0)
        im = pybullet.getCameraImage(self.width, self.height, viewMatrix=view_matrix,
                                     projectionMatrix=projection_matrix, renderer=pybullet.ER_BULLET_HARDWARE_OPENGL, shadow=1)
        rgba = im[2]
        return rgba
